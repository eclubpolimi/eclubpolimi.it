...
export const GET: APIRoute = async ({ request }) => {
    try {
        ...
        if (!res.ok) {
            console.error('[public-events] Google Calendar API error:', res.status);
            return new Response(JSON.stringify({ error: 'Failed to fetch calendar events' }), { status: 502 });
        }
        ...
        console.error('[public-events] Error fetching calendar events:', error);
